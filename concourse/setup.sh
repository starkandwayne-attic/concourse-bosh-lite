...
if [[ "$(which jq)X" == "X" ]]; then
  ...
  curl -s https://api.github.com/repos/concourse/concourse/releases/latest | jq -r ".assets[].browser_download_url" | xargs -L1 bundle exec bosh -t concourse-bosh-lite upload release
fi
